...
const estilo={ color: ' #EC410B'};
  ...
  return (
    <div><b>
        <Router>
  <Layout >
    <Header className="header"  style={{background:'#DCD212'}}>
      <div className="logo" />
   
      <Menu style={{ background:'#DCD212' }} mode="horizontal" defaultSelectedKeys={['2']}>
      ...
      <div className="float-right">  <a href="http://52.143.244.225:8080/auth/realms/INFANCIA/protocol/openid-connect/logout?client_id=security-admin-console&redirect_uri=http%3A%2F%2Flocalhost%3A3000/index"><Button style={{ borderWidth:1,
    borderColor:'rgba(0,0,0,0.2)',
    alignItems:'center',
    justifyContent:'center',
    width:50,
    height:50,
    backgroundColor:' #EC410B',
    borderRadius:100,
  }}><PoweroffOutlined /></Button></a>
  </div>
      </Menu>
    </Header>
    <Content style={{ padding: '0 50px', background: 'black', color:'white'}} >
      <Breadcrumb style={{ margin: '16px 0',background: 'black', color: 'white' }}>
        <Breadcrumb.Item>Home /</Breadcrumb.Item>
        <Breadcrumb.Item >SICOIN</Breadcrumb.Item>
      </Breadcrumb>
      <Layout className="site-layout-background" style={{ padding: '24px 0',background: 'black' }}>
        <Sider className="site-layout-background" width={200} >
          <Menu
            mode="inline"
            defaultSelectedKeys={['1']}
            defaultOpenKeys={['sub19']}
            style={{ height: '100%', color: '#21B30D'}}
            id="example1"
